...
# --------------------------------------------------------------------------------
# CORE LOGIC (Originally editorial_tracker.py)
# --------------------------------------------------------------------------------
...
def init_db(engine) -> None:
    try:
        ...
        # Pass to allow app to start even if DB is flaky, though endpoints will fail
        pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    list_parser.add_argument(
        "--sort",
        choices=["due_date", "venue", "title", "reference_id"], 
        default="due_date",
...
def main() -> None:
    # Only run CLI if executed directly or if sys.argv implies CLI usage (a bit hacky for mixed use)
    # But for our shim, we call main() explicitly.
    parser = build_parser()
    
    # If no args provided (e.g. run via uvicorn), don't parse
    if len(sys.argv) == 1:
        return
...
# Check if public directory exists before mounting to avoid errors if path is wrong
if os.path.exists("public"):
    app.mount("/", StaticFiles(directory="public", html=True), name="public")
elif os.path.exists(os.path.join(os.path.dirname(__file__), "../public")):
    # Fallback for Vercel structure if needed
